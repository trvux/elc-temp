...
type ServiceInsert = Insert<"services">;
...
export class SupabaseServiceRepository implements ServiceRepository {
  ...
  async getAll(options?: ServiceFilter): Promise<Service[]> {
  ...
  private applyFilters(query: any, options?: ServiceFilter) {
    let q = query;
  ...
  private mapToDomain(row: ServiceRow): Service {
    return {
      id: row.id,
      title: row.title,
      slug: row.slug,
  ...
  private handleError(error: unknown, context: string): never {
    ...
    console.error(`[SupabaseServiceRepository][${context}] Error:`, error);
