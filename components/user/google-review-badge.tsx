"use client";

import { Star, StarHalf } from "lucide-react";
import { cn } from "@/lib/utils";

interface GoogleReviewBadgeProps {
  rating?: number;
  count?: number;
  className?: string;
  showText?: boolean;
}

export function GoogleReviewBadge({
  rating = 4.8,
  count = 1248,
  className,
  showText = true,
}: GoogleReviewBadgeProps) {
  // Generate stars based on rating
  const fullStars = Math.floor(rating);
  const hasHalfStar = rating % 1 >= 0.5;

  return (
    <div className={cn("inline-flex flex-col gap-1.5 p-2 rounded-xl transition-all duration-300 hover:bg-muted/30 group", className)}>
      <div className="flex items-center gap-2">
        {/* Google "G" Logo - Simplified SVG */}
        <div className="flex items-center justify-center w-5 h-5 bg-white rounded-full shadow-sm border border-border/10 overflow-hidden">
          <svg viewBox="0 0 24 24" className="w-3.5 h-3.5">
            <path
              fill="#4285F4"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="#34A853"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"
            />
            <path
              fill="#EA4335"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.66l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
        </div>

        <div className="flex items-center gap-1">
          <span className="text-sm font-bold text-foreground/90">{rating.toFixed(1)}</span>
          <div className="flex items-center">
            {[...Array(5)].map((_, i) => {
              if (i < fullStars) {
                return (
                  <Star
                    key={i}
                    className="w-3.5 h-3.5 fill-[#FBBC05] text-[#FBBC05]"
                  />
                );
              }
              if (i === fullStars && hasHalfStar) {
                return (
                  <StarHalf
                    key={i}
                    className="w-3.5 h-3.5 fill-[#FBBC05] text-[#FBBC05]"
                  />
                );
              }
              return (
                <Star
                  key={i}
                  className="w-3.5 h-3.5 fill-muted text-muted"
                />
              );
            })}
          </div>
        </div>
      </div>

      {showText && (
        <p className="text-[11px] font-medium text-muted-foreground/80 tracking-wide uppercase">
          {count.toLocaleString()} đánh giá trên Google
        </p>
      )}
    </div>
  );
}
