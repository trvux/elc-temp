...
async function generateReport() {
  ...
  let md = '# Báo cáo Preview SEO (100 Sản phẩm)\n\n';
  md += '| Sản phẩm | Preview Snippet trên Google |\n';
  md += '| :--- | :--- |\n';

  products.forEach(p => {
    const snippet = generateProductSmartDescription(p);
    md += `| **${p.name}** | ${snippet} |\n`;
  });

  fs.writeFileSync('scratch/seo_preview_report.md', md);
  console.log('Report generated at scratch/seo_preview_report.md');
}
